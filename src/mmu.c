#include "state.h"
#include <raylib.h>

const byte bios[0x0100] = {
    };

void initialize_mmu(State *state) {
  TraceLog(LOG_DEBUG, "Initializing MMU");

  MMU mmu;
  mmu.bios_disabled = false;
  mmu.cartridge_size = 0;

  for (unsigned long i = 0; i < sizeof(mmu.bios); i++) {
    mmu.bios[i] = bios[i];
  }

  state->mmu = mmu;

  TraceLog(LOG_DEBUG, "MMU initialized");
}

byte read_byte_from_mmu(MMU *mmu, word address) {
  if (address < 0x0100) {
    return mmu->bios[address];
  }
  return 0;
}

void write_byte_to_mmu(MMU *mmu, word address, byte value) {
  if (address < 0x0100) {
    mmu->bios[address] = value;
  } else if (address >= 0x8000 && address <= 0x9FFF) {
    mmu->vram[address - 0x8000] = value;
  } else if (address >= 0xC000 && address <= 0xDFFF) {
    mmu->wram[address - 0xC000] = value;
  } else if (address >= 0xFF80 && address <= 0xFFFE) {
    mmu->internalRam[address - 0xFF80] = value;
  }
}
